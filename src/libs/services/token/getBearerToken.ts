...
import { BASIC_TOKEN_AP, BASIC_TOKEN_EL, BASIC_TOKEN_EU, BASIC_TOKEN_US } from '@libs/constants'
...
export async function getBearerToken(apiType: APIType) {
  ...
  if (!(url != null && basicToken != null)) { throw new Error(`url or basic token missing for ${apiType}`) }
