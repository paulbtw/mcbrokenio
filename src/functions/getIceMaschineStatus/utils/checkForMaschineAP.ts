...
const logger = new Logger('checkForMaschineAP');

export const checkForMaschineAP = async (
  bearerToken: string,
  ...
  try {
    const response = await axios.get(
      `https://ap-prod.api.mcd.com/exp/v1/restaurant/${nationalStoreNumber}?filter=full&storeUniqueIdType=NSN`,
      {
        headers: {
