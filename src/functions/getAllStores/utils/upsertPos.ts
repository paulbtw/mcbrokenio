...
const generateValueString = (posArray: Pos[]) => {
  const valuesString = posArray.map((pos) => {
    return `('${pos.nationalStoreNumber}', '${pos.name.replace(new RegExp(/"|'/g), '').trim()}', '${pos.restaurantStatus.replace(new RegExp(/"|'/g), "").trim()}', '${pos.latitude}', '${pos.longitude}', '${pos.country}', ${pos.hasMobileOrdering}, current_timestamp)`;
  });
...
export const upsertPos = async (arr: Pos[], connection: Connection) => {
  ...
  await connection.query(`INSERT INTO pos ("nationalStoreNumber", name, "restaurantStatus", latitude, longitude, country, "hasMobileOrdering", "updatedAt") VALUES ${valuesString} ON CONFLICT ("nationalStoreNumber") DO UPDATE SET "updatedAt" = current_timestamp`);
