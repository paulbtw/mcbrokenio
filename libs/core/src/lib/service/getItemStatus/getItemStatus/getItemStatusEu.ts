import { ItemStatus as ItemStatusPrisma, Pos } from '@prisma/client';
import { Logger } from '@sailplane/logger';
import { ICountryInfos } from '../../../types';
import axios, { AxiosError } from 'axios';
import { RestaurantInfoEuResponse } from '../../../types/responses';
...
export async function getItemStatusEu(
  pos: Pos,
  ...
  try {
    ...
    const { data } = await axios.get<RestaurantInfoEuResponse>(
      `https://eu-prod.api.mcd.com/exp/v1/restaurant/${pos.nationalStoreNumber}?filter=full&storeUniqueIdType=NatlStrNumber`,
      {
        headers: {
    ...
    if (!outageProductCodes || !outageProductCodes.length) {
      return null;
    }
